...
def set_criterion(cfg, dataset):
    Net_config = {'VOC': cfg.model.voc_config, 'COCO': cfg.model.coco_config}
    _Net_config = Net_config[dataset]
    return MultiBoxLoss(_Net_config.num_classes,
...
def get_network(build_net, cfg, dataset, phase='train'):
    Net_config = {'VOC': cfg.model.voc_config, 'COCO': cfg.model.coco_config}
    _Net_config = Net_config[dataset]
    net = build_net(phase, cfg.model.input_size, _Net_config)
    return net
